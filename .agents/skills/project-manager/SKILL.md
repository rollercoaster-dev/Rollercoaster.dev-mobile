...
4. Maintain existing PRs and reservations, then **dispatch eligible issue workers** as below while capacity allows. Progress the wider relevance audit after the dispatch decision; a large backlog is not a reason to leave approved Next work idle. Review pending Dependabot updates using `dependabot-review`; approve every update that passes its compatibility, validation, independent-review, and no-automatic-merge checks in the same run. Dependency approval is already authorized. Read governing instructions from fresh main or the installed runtime, not stale PR checkouts.
...
Five slots = tracked open issue-work PRs + active reservations + closed-unmerged holds. Dependabot and verified release-please PRs are separate. At five, DO NOT start another issue, even to prepare code without a PR. Dispatch multiple **independent** issues when agent capacity and PR slots allow; avoid assigning overlapping files to simultaneous workers. The guard makes each claim atomically and enforces the five-slot cap. Never merge, enable auto-merge, enqueue a merge, or approve your own PRs.

Before claim, revalidate the selected issue against freshly fetched main and refresh its audit if needed. Run `pm.py status` for `next_auto_issue`; assess that issue and record its verdict, then call `claim ISSUE`. Repeat for another independent candidate while slots and agents are available. Only a **manual** queue is frozen to its approved board order. A newly opened or changed unrelated issue does not invalidate an already audited candidate. If the first candidate is still needed but cannot proceed, record the blocker and ask Joe to change its board status or priority; do not fabricate a negative verdict to skip it.
