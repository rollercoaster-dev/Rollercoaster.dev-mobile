...
4. Progress the issue audit or maintain the approved queue as below. Review pending Dependabot updates using `dependabot-review`; approve every update that passes its compatibility, validation, independent-review, and no-automatic-merge checks in the same run. Dependency approval is already authorized and does not wait for issue-queue approval. Read governing instructions from fresh main or the installed runtime, not stale PR checkouts. Prioritize user blockers and maintain existing issue PRs before starting a new one.
