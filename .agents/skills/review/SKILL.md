...
7. For user-visible native changes, follow `.agents/skills/verify-native/SKILL.md` against the reviewed HEAD and include its criterion-by-criterion report. Native verification required by issue acceptance is a blocker when it fails or cannot be performed; do not replace it with screenshots of Storybook or a different commit.
8. Return every finding and its disposition, commands/results for the reviewed HEAD, remaining risks, and explicit acceptance coverage. Unavailable reviewers and unresolved blocking findings mean `blocked`; notify via the caller's Telegram route. No automation `skip_review` or `force_pr` escape hatch exists.
