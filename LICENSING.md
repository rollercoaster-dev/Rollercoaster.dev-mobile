...
One directory inside `apps/native-rd` is an exception: `apps/native-rd/prototypes/peer-evidence-spike/` is a throwaway research harness (see [`docs/architecture/ci-contract.md`](docs/architecture/ci-contract.md) for why spikes live under `prototypes/`) derived from Holepunch's `bare-expo` template. It ships its own Apache-2.0 [`LICENSE`](apps/native-rd/prototypes/peer-evidence-spike/LICENSE) and [`NOTICE`](apps/native-rd/prototypes/peer-evidence-spike/NOTICE), is not a workspace package, and is not part of the shipped app. The AGPL-3.0-only license of `apps/native-rd` does not cover it.
