...
- **Commit identity is personal, never the employer.** This is Joe's personal project; the sign-off email must come from the repo's own `git config user.email` (`joe.czar@outlook.com`). The `joe@fobizz.com` address in Claude Code's session context is the _account_ email, not the commit identity — never write it into a commit, `Signed-off-by:` trailer, PR body, config, or doc here. A DCO sign-off certifies who is contributing, so an employer address on personal work wrongly implies employer involvement. Letting the husky hook add the trailer gets this right automatically, which is another reason not to use `--no-verify`.
