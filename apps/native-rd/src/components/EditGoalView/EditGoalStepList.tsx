...
export interface EditGoalStepListProps {
  ...
  /**
   * Fires when a row drag starts / ends, so the host can disable its
   * ScrollView for the duration — a native ScrollView isn't in RNGH's gesture
   * tree, so otherwise it wins the pan and the row never moves.
   */
  ...
  // --- In-row timing editor (#576), forwarded to whichever row owns the id. ---
...
export function EditGoalStepList({
  ...
  useEffect(() => {
    onDragStateChange?.(drag.isDragging);
  }, [drag.isDragging, onDragStateChange]);
          ...
          return (
            <View key={step.id}>
              <EditGoalStepRow
                ...
                editTimingUnsetA11yLabel={editTimingUnsetA11yLabel}
                editTimingSetA11yLabel={editTimingSetA11yLabel}
                isTimingExpanded={expandedTimingId === step.id}
                onCommitTiming={
                  onCommitTiming
                    ? (next) => onCommitTiming(step.id, next)
                    : undefined
                }
                onClearTiming={
                  onClearTiming ? () => onClearTiming(step.id) : undefined
                }
                onCollapseTiming={
                  onCollapseTiming ? () => onCollapseTiming(step.id) : undefined
                }
                timingNow={timingNow}
                timingLocale={timingLocale}
                timingCopy={timingCopy}
