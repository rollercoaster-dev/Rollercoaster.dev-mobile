...
describe("bakePNG / unbakePNG roundtrip", () => {
  ...
  it("re-baking replaces the existing credential — only one iTXt chunk with the keyword exists after re-bake", () => {
    ...
    const recovered = unbakePNG(
      bakedTwice,
    ) as unknown as typeof updatedCredential;
  ...
  it("roundtrips a JWS string credential (compact serialization path)", () => {
    ...
    const jws =
      "eyJhbGciOiJFZERTQSJ9.eyJzdWIiOiJkaWQ6a2V5OnRlc3QifQ.AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA";
