...
| Dependency display        | Walkthrough (2026-06-17) **settles the wording**: "after / depends on" (internal) and "waiting on … expected ⟨date⟩" (external), never "blocked by" — read as informative, no lateness register, met internal deps read as quiet history. **Treatment stays open**: inline is cleanest (clear ⏳, stacks two markers calmly); chip clips external markers on the 360px card; connector draws the relation but reads graph-like and uses an opaque glyph (⤧). All three kept live per Joe's call; overflow + glyph/tie-line defects to fix before the dev-flag rung. | C prototype (Stage 2)                    | Joe   | Open   |
