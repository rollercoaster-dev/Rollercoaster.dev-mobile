...
| S3  | **Bottom Nav: demote + new goal, resolve secondary tensions (S5 owns resume)** | `app:native-rd` `enhancement` `dep:blocked` `order:3` `size:m` `priority:medium` `needs:design`   | Still a real redesign (`Bottom Nav Cool Directions.dc.html` exists) but **quieter**: demote the `+ new goal` FAB out of its loud central slot (rare action), resolve the brief's secondary tensions (Badges as a quieter destination vs. equal peer; the detached/afterthought Settings pill; whether the label-morph + half-lifted pill earn their complexity vs. the low-load mandate; keep the bar stable across contexts), keep ≤3 destinations (already true today), honor the `PILL_LIFT` / `useTabScreenContentInset` coupling (`EvidenceDrawer` reads it). **OUT:** a loud global "resume" control — **S5's cockpit owns it**; S3 and S5 must be designed coherently. **AC:** `+ new goal` demoted from the loud central slot; secondary tensions resolved; ≤3 destinations; inset/lift not broken; no duplicate resume control. |
...
| S5  | **Goals + Badges: cockpit + wall-of-proof**                                    | `app:native-rd` `enhancement` `dep:blocked` `order:3` `size:m` `priority:low`                     | Goals = momentum cockpit (progress ring + one Start/Resume — **this owns "resume my current goal,"** so design coherently with S3); Badges = dark wall-of-proof (count + latest + gallery); both empty states. **Token risk:** the wall uses a **full-black surface** that is NOT a modeled token. Render it across all 7 themes; if full-black doesn't adapt cleanly (dark modes, dyslexia cream bg, autismFriendly muted/reduced noise), add a per-theme `badge-wall` (or similar) surface token instead of hardcoding `#000`. Discovered-during-implementation — don't pre-author the token; V1's 7-theme walkthrough is the backstop, and a token addition loops back to F2-style contract work. Ref `Goals + Badges C Prototype.dc.html`.                                                                                           |
...
- **S3 and S5 are a coherent pair:** S5's cockpit owns the global "resume current goal"; S3 must not duplicate it with a loud resume control. Note this in both issue bodies.
