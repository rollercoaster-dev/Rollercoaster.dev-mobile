...
- [x] Every fixture string passes a banned-phrase check: none of the `BRAND_LANGUAGE.md` forbidden terms appear in the German output (e.g. "special needs", "differently abled", "high/low functioning", exit-aside phrasings). _(Assertion #2 via `not-icontains-any` with English + researcher-drafted German equivalents.)_
...
| `llm-rubric` JSON-pinned judge output             | Self-review HIGH #3: judge timeout/429/content-filter could silently retry into degraded response; rubric is binary but threshold is numeric `0.5`. Audit raw judge responses in the HTML report after first live run; pin to `{"pass": boolean, "score": number, "reason": string}` if drift observed.       | Discovery log after first run |
| Rubric threshold / binary-prompt reconciliation   | Self-review MEDIUM #7: threshold `0.5` is harmless given binary "PASS/FAIL" prompt today, but becomes a footgun if the rubric is later rewritten to scored output without re-tuning. Either drop the threshold or rewrite rubric to score each criterion 0–1 and average. Folds into the HIGH #3 audit above. | Discovery log after first run |
| German banned-phrase expansion candidates         | Self-review HIGH #4 suggested adding `oder überspring es`, `oder lass es bleiben`, `oder eben nicht` to the substring blocklist. Deferred until first eval surfaces whether any candidate actually produces these constructions; speculative additions risk false-positive blocking on legitimate fixtures.   | Discovery log after first run |
