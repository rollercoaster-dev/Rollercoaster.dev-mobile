...
- [2026-09-03 12:05] Live runs done. User chose the project account `rollercoaster-dev.bsky.social` over D6's throwaway (0 posts, few followers, evidence lands in Rollercoaster.dev's own repo). Handle `rollercoaster.dev` is NOT set up as an atproto handle — no `_atproto` DNS TXT, no `.well-known/atproto-did` — so the `.bsky.social` handle is the real one.
- [2026-09-03 12:05] `searchPosts` returns 403 anonymously on `public.api.bsky.app`; must go through the authenticated session (PDS proxies to the AppView). `getAuthorFeed` is fine anonymously.
- [2026-09-03 12:05] Identity-creation task reinterpreted: the account predates the spike, so the signup was not observed. The PLC audit log (2 Bluesky-held rotation keys, 1 PDS-held signing key) is the custody evidence instead; README says so.
