...
## Findings (research, 2026-07-02)

Two questions came up while reviewing the shipped #415 components. Recorded here so #416 (screen assembly) inherits the context.

### A. "The section background doesn't change with the theme"

**Not a component bug — a wiring gap that is correct for #415.** `SettingsSection`'s chrome (`rows.backgroundColor: theme.colors.backgroundSecondary`, `SettingsSection.styles.ts:23`) is styled with unistyles reactive tokens, which always resolve to the **single globally-active** theme (`UnistylesRuntime.themeName`). `SettingsThemeSection`/`ThemeSwatchRail` are controlled (D1) and never call `setTheme` — tapping a swatch only updates local `selectedThemeId` state. The only thing that recolors is `ThemeSampleCard`, because it reads `themes[selectedThemeId]` **statically** as inline styles (`ThemeSampleCard.tsx:22-23`), bypassing the active-theme binding.

- **#416 fix (already its job):** wire `onSelect` → `useThemeContext().setTheme` (same call `ThemeSwitcher.tsx:157` makes today). A tap then flips the global theme → the section bg and the whole app recolor, and the sample card matches. No change needed to the #415 components.
- **`AllThemesMatrix` caveat:** unistyles holds one active theme, so the 7 sections' inner `rows` cards all render the _active_ theme's bg; the per-theme look is faked by the story's outer wrapper `backgroundColor` (`SettingsThemeSection.stories.tsx:46`). Honest for the sample card, misleading for the section chrome — don't read the matrix as proof the chrome themes correctly.
- Documented inline in `SettingsThemeSection.tsx`'s JSDoc so the next reader doesn't re-diagnose it.

### B. Backed-but-unsurfaced user settings (answers "what real settings could this screen add")

Inventory of `userSettings` (`db/schema.ts:164-173`) vs. what the Settings screen exposes. Density and theme are real and covered by #415. The rest, ranked by readiness — **none of these are in #415's scope; this is scoping input for #416 / future issues, not a request to expand this PR:**

| Field                                              | State                                                                                                                                                                                                                                                                                     | Verdict                                                                                                                           |
| -------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------- |
| `animationPref` (Reduce Motion: full/reduced/none) | Fully honored by 10+ components via `useAnimationPref`; `setAnimationPref` exists with **zero production callers** (only test mocks). Today only OS reduce-motion / autism-friendly theme can drive it.                                                                                   | **Tier 1 — strongest next real row.** Reuses the `SettingsDensityRows` radiogroup pattern verbatim. On-brand for an ND-first app. |
| `hasSeenWelcome` (Replay welcome)                  | `useFirstLaunch` reads it; `markWelcomeSeen` sets `1`; **no reset path exists**. Already drawn in the `SettingsFrame` prototype's Onboarding section.                                                                                                                                     | **Tier 4 — needs a small `resetWelcome` mutation.** Low effort.                                                                   |
| Language (English / Deutsch)                       | Real, substantial `de` translations exist; `supportedLngs: ["en","de","pseudo"]`. But `LanguagePicker` is `__DEV__`-gated + only toggles pseudo, and language is read from device locale each launch (`i18n/index.ts:191`) with **no persistence**.                                       | **Tier 2 — backable but needs a persisted `language` field / detector** to survive restart (userSettings has none today).         |
| `focusTimelineHidden` (hide focus timeline)        | Wired via `useFocusModePrefs`, already toggled on `FocusModeScreen`.                                                                                                                                                                                                                      | Real, already has a home. Mirror in Settings only if discoverability matters.                                                     |
| `fontScale` (text size 80–150%)                    | Schema field + `updateUserSettings` write-validation exist (`queries.ts:1519-1524`), but **nothing reads `settings.fontScale`**. The `fontScale` in BadgeCard/GoalEvidenceCard is `PixelRatio.getFontScale()` (OS-level, unrelated). Overlaps the `largeText`/`lowVision` theme variants. | **Dead field — do not build a "text size" row on it** without first wiring a consumer + deciding the theme-variant overlap.       |
| `keyId`                                            | Internal Ed25519 keypair reference.                                                                                                                                                                                                                                                       | Not a user-facing setting.                                                                                                        |

**No backing at all (would be fake rows — don't invent):** notifications/reminders (no notification system), account/sync/data-export (Evolu is local-first).

A short pointer to this inventory is left inline in `SettingsDensityRows.tsx`.
