...
| `components/StepCard/` (`StepCard.tsx`, `StepOverviewCard.tsx`, `StepCardTopBand.tsx`, `index.ts`) deletion                                                                                    | **Newly story-only after this sweep, but must not be deleted.** `CardCarousel.stories.tsx` was `StepCard`'s only importer outside its own directory, so these four are now reachable only from their own story/test/barrel — _except_ that open issue **#537** (#533 F3) is chartered to port `StepOverviewCard` and render `StepCardTopBand`; both are built and translated in en/de/pseudo, and the Storybook-only mount is the deliberate parking mechanism, not neglect. Deleting them turns #537 from wiring into a rebuild. The directory also cannot go wholesale: `StepCardEvidenceCapture.tsx` is live via `FocusCurrentTaskCard.tsx:6`. | Revisit only after #537 lands. Note #537's body still asserts `StepCard` is imported by `CardCarousel.stories.tsx` — stale as of this branch. |
