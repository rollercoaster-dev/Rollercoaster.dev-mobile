...
| Item                                                                                                                                                                                                       | Reason it is not fixed in this slice                                                                                                                                                                                                                                                                                                                                |
| ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `userSettings.focusTimelineHidden` is now a dead column — deleting `useFocusModePrefs` removed its only reader, but the column, `updateUserSettings`'s field, and its `validateIntField` guard all remain. | Evolu schemas are additive, so the column itself cannot be dropped; retiring the write path is a settings-layer cleanup with its own test surface, unrelated to mounting the card. Harmless meanwhile: nothing writes it either.                                                                                                                                    |
| Setting a step aside / picking it back up fire no `AccessibilityInfo` announcement, while complete and reopen do (`a11y.stepCompleted` / `a11y.stepUncompleted`).                                          | No copy exists for those two transitions in any of the three locale bundles, and #467 already owes a call on which focus transitions warrant an announcement (see the child→parent question in the handoff above). Inventing two strings here would pre-empt that decision and land untranslated German.                                                            |
| `canCompleteStep`'s "at least one planned type captured" floor stays looser than the card's "every planned type captured" reveal, so a step the card still shows as unfinished can pass the DB gate.       | Deliberate and now documented on the function (#497 D1). Tightening the data layer to match the card would change the gate for every future caller — a decision that belongs with #497's contract, not with this mount. `FocusModeScreen` is the only `completeStep` caller today, and it reaches the gate only after the card's stricter reveal, so nothing leaks. |
