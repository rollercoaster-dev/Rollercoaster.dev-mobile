...
- `apps/native-rd/docs/plans/dev-plans/issue-284-ns-key-parity.md`: this plan document.
...
- [ ] Add `formatFailure(match: PrefixedMatch, reason: string): string` helper (exported). Returns `${match.file}:${match.line} → t(${match.raw}) — ${reason}` — exactly the format documented in D7 (`raw` already carries the quotes/backticks from `scanFilePrefixed`).
...
src/screens/SomeScreen.tsx:55 → t(`badgeDesigner:colour.options.${id}`) — path "colour.options" not found in namespace "badgeDesigner"
src/screens/SomeScreen.tsx:60 → t(`badgeDetail:earned.${x}`) — path "earned" resolves to a string leaf, not an object (dynamic suffix requires an object parent)
```
