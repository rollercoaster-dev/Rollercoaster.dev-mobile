...
| D5  | `i18n:lint-source` script invoked with `bun run scripts/i18n/lintSource.cli.ts`                                | Compile to JS first; use a Bun entrypoint; use `tsx`             | `bun run <ts-file>` is the established pattern in this repo (see `gen:pseudo`, `verify:badge`, `release-notes:*` in `package.json`). No compilation step needed. The `.cli.ts` split (see Discovery Log) keeps `lintSource.ts` jest-importable; the CLI wrapper owns the `import.meta` bootstrap.                                                                      |

## Affected Areas

- `apps/native-rd/scripts/i18n/lintSource.ts`: new file — pure linting logic (exports `lintEnDir`, check functions, `formatFinding`)
- `apps/native-rd/scripts/i18n/lintSource.cli.ts`: new file — thin CLI wrapper that owns the `import.meta` bootstrap and the unconditional exit-0 contract (split from `lintSource.ts` so jest+Hermes can import the pure module; see Discovery Log)
- `apps/native-rd/scripts/i18n/__tests__/lintSource.test.ts`: new file — tests for all three checks
- `apps/native-rd/package.json`: add `"i18n:lint-source": "bun run scripts/i18n/lintSource.cli.ts"` to `scripts`
...
## Lessons (worth keeping for the next i18n / scripts/ author)

1. **`import.meta` in `scripts/*.ts` is only safe if no jest test imports it.** `generate-pseudo-locale.ts` gets away with `import.meta.dir` because nothing under `__tests__/` imports it. The moment a test reaches a module that uses `import.meta`, jest's babel pipeline (babel-preset-expo, Hermes target) errors with "`import.meta` is not supported in Hermes. Enable the polyfill `unstable_transformImportMeta` in babel-preset-expo." For any `scripts/*.ts` that should be unit-testable, split the CLI bootstrap into `<name>.cli.ts` and keep `<name>.ts` pure. Bonus: this is a cleaner architecture independent of the test concern — "make it testable" pushed the file toward better separation, not worse.

2. **Plan-time hedging + run-time resolution beats guessing in either direction.** Ambiguity #1 ("will the placeholder-conflict heuristic flood?") was deliberately left open, with the implementer instructed to run the linter against real `en/` early and observe signal-to-noise. The answer (3 findings, all genuine conflicts) decided the question with one command — no heuristic tightening needed. This pattern — name the unknown, defer the decision to first contact with real data — is the right move when the alternative is over-engineering for a noise problem that may not exist.

3. **`tsconfig.scripts-test.json` carries a hidden coupling: it must include type packages for everything its tested files transitively import.** PR #156 set `types: ["jest"]` because at that time the only test (`placeholderGuard.test.ts`) imported a script with zero node-module usage. The moment `lintSource.test.ts` imported `lintSource.ts` (which uses `node:fs` / `node:path`), the test tsconfig needed `"bun"` (or `"node"`) added. Watch for this when adding the next script test that touches the filesystem or `node:` modules — the failure mode is `Cannot find name 'node:fs'` and `implicitly has an 'any' type` errors that look unrelated.
