...
Phases 3A and 3B are skipped as standalone candidates. (Post-3C, the prototype's
shared top band — including 3A's parent band + part-numbering — was folded into
candidate C at Joe's request; see D9/D10.)
...
| D9  | Adopt the prototype's **full unified top band** on every step card (chosen 2026-06-21, post-Phase-3C) | (a) overview-band only; (b) keep the shipped in-body meta row + quiet "↳ in [parent]" line | Joe reviewed the prototype and asked for "full proto." A pinned, bordered header strip replaces the in-body meta row: plain `N of M` for flat/overview cards (`· Overview` suffix on overview), purple `↳ [parent] · part N of M` for child leaves. This **overrides** the Phase 3C plan text that kept leaves on the quiet `↳ in` line, folds the parent-band + part-numbering idea from the skipped Phase 3A into candidate C, and drops the now-dead `common:stepCard.parentContext` key. Child cards no longer show a global `N of M` (the band carries `part N of M`); carousel position stays on the MiniTimeline + ProgressDots. |
| D10 | Overview foot stays **consistent with leaf cards** — no "Open next part" action (chosen 2026-06-21)   | Prototype's "Open next part →" foot button                                                 | Joe: "keep the footer like the rest." The overview foot is the mark-complete invite (Checkbox) once all parts are done, otherwise the quiet `partsPendingPrompt` — the same shape as a blocked leaf foot. Parts are reached by swiping, like every other card; the bespoke jump button + `onOpenNextPart` wiring were removed.                                                                                                                                                                                                                                                                                                          |
...
**Phase 3C addendum — full-prototype header (2026-06-21, D9/D10):**

- [2026-06-21] After the first 3C pass, Joe asked for the prototype's **full
  header**: a pinned `StepCardTopBand` on every step card (shared component).
  Plain band (`N of M`, `· Overview` on the overview) for flat/overview cards;
  purple `↳ [parent] · part N of M` band for child leaves. This moved the meta
  out of the scrollable body, removed the quiet `↳ in [parent]` line and its
  now-dead `common:stepCard.parentContext` key, and folded the Phase-3A parent
  band + part-numbering into candidate C (threaded `partIndex`/`partTotal` from
  `FocusModeScreen` via a `partInfoByChildId` map).
- [2026-06-21] **Child cards lost the global `N of M`** — the band shows
  `part N of M` instead (prototype fidelity). The #292 sub-step snap tests that
  used `getByText("N of M")` as the "current card" proxy now assert the band
  text (`band.childContext`, on-screen ⇒ current, since off-screen cards are
  hidden); carousel position is still covered by the MiniTimeline node-width
  tests.
- [2026-06-21] **Overview foot kept like the rest (D10)** — the "Open next
  part →" button and its `onOpenNextPart` wiring were removed; the foot is the
