...
- `apps/native-rd/src/i18n/resources/en/focusMode.json`: new `progressStrip` and `parked` key groups. English is the only hand-authored locale. `pseudo/focusMode.json` is regenerated **in this PR** via the local `bun run gen:pseudo` script (the `locale-parity` test gates on en↔pseudo parity, so it must land here — it is not the bot's job). `de/focusMode.json` is synced by the `.github/workflows/i18n-sync.yml` bot, which runs **on the PR** whenever `en/**` changes and pushes a `de/` commit back to the branch — so a `de/` diff appearing in this PR is expected. Do not hand-translate `de/` (the bot overwrites it).
...
- [2026-07-02] Follow-up to the entry above: the i18n-sync bot ran on the PR
  (it triggers on `pull_request` when `en/**` changes, not after merge) and
  committed `de/focusMode.json` back to the branch (`ce0827e6`,
  `github-actions[bot]`). So `de/` was accurately "untouched" at 15:10, but the
  PR now carries a bot-authored `de/` diff by design — that is the sync working,
  not a hand-translation. (Affected Areas above corrected to match.)
- [2026-07-02] PR #457 review fixes (Copilot):
  - `FocusProgressStrip.tsx` `now`: hardened the percent so 0%/100% are reserved
    for `done === 0` / `done === total`. `Math.round` alone let a partial goal
    read as complete (199/200 → 99.5 → 100) — the bar/a11y `now` would say
    "done" while the label said 199/200. Partial fractions now clamp to [1, 99].
  - `FocusProgressStrip.test.tsx`: added a `CLAMPED` `test.each` locking the
    `< 0` and `> total` clamps plus the 199/200 (no false 100) and 1/200 (no
    false 0) endpoint cases.
  - The "phantom a11y button when `onPress` is omitted" finding was already
    fixed in `b08f8ee0` (handler-less strip renders a plain `View`; covered by
    the "renders no phantom button" test) — no further change.
