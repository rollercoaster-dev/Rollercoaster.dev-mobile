...
| D8  | Allowlist = "no `\p{L}` and no `\p{N}`" (Unicode letter/digit test), plus a child-only `JSXExpressionContainer` string-literal visitor           | Keep `codePointAt > 127` + single-char branches; visit only `JSXText`           | Review found `>127` exempted all non-Latin script copy and `{"literal"}` children bypassed the rule. The category test flags letters in every script while still allowing punctuation/separators/PUA/emoji glyphs (the real icon usage in this repo), and is simpler than the three branches it replaces. Prop literals (`label={"x"}`) excluded by checking the container's parent is a JSXElement/JSXFragment.                            |
...
- [2026-05-27] REVIEW FIX (D8): review agents (silent-failure-hunter HIGH, code-reviewer MEDIUM ×2) found two masking holes. (a) The `codePointAt > 127` branch + the single-char branch exempted all non-Latin script copy (你好, 是) as icon glyphs — open question 3 from research, escalated to a fix. (b) `{"literal"}` string children (JSXExpressionContainer) bypassed the rule. Replaced the `>127`/single-char heuristics with one Unicode test — allow only strings containing no `\p{L}` and no `\p{N}` (punctuation/separators/PUA/emoji glyphs); flag letters in any script + number+unit strings. Added a child-only `JSXExpressionContainer` visitor (props like `label={"x"}` stay out of scope). Surfaced one new offender: `NewGoalModal.tsx` literal `X` close glyph (silenced; could align with the `✕` viewer-modal convention). The 7 existing `{"\uXXXX"}` icon children are all symbols/emoji and remain allowed. Tests 17 → 26.
