...
- [ ] `cd apps/native-rd && APP_VARIANT=development npx expo config --json | jq '{a:.android.package, i:.ios.bundleIdentifier}'` → `android.package` = `dev.rollercoaster.app.dev`, `ios.bundleIdentifier` = `dev.rollercoaster.app` (iOS always base — see D3).
...
`android.package` to `dev.rollercoaster.app.dev` **only** when `APP_VARIANT=development`
(set by the local run scripts). `ios.bundleIdentifier` always stays base (D3). Every
other path — including all EAS builds — resolves to the base `dev.rollercoaster.app`. Result:
...
| D3  | Only `android.package` gets the `.dev` suffix locally; `ios.bundleIdentifier` always stays base | Symmetric `.dev` on both platforms                         | The friction (`INSTALL_FAILED_VERSION_DOWNGRADE`) is Android-only — iOS has no downgrade error class, so a `.dev` iOS bundle id buys nothing functional. It is also **not** harmless: it breaks the local Maestro E2E suite, which hardcodes `dev.rollercoaster.app` in `scripts/run-e2e.sh` (UserDefaults seeding) and every `e2e/flows/*.yaml` `appId`, and it would diverge from the ASC/TestFlight record. Android-only avoids all of that with zero E2E/iOS-tooling edits. (Revised 2026-06-14 after PR review flagged the E2E collision.) |
...
- **Config resolution check** (runnable here — no native build, no EAS):
  - `cd apps/native-rd && APP_VARIANT=development npx expo config --json | jq '{a:.android.package, i:.ios.bundleIdentifier}'` → `android.package` = `…​.dev`, `ios.bundleIdentifier` = base (D3).
