...
**Commit**: `fix(edit-goal): stop step/sub-step rows crushing the title on narrow screens` (SHA 85803730)
**Status**: ✅ Landed — surfaced by review screenshots on 2026-07-07 (see Discovery Log). Verified at 340px in Storybook: the two-evidence-type step ("Gather references"), previously collapsing to one char/line, now keeps its full title on line 1 with the controls wrapped to a right-aligned second line; every sub-step row behaves the same; the header/GOAL card no longer clip.
...
- [x] `EditGoalView.styles.ts`: `rowMain` and `subStepRow` gained `flexWrap: "wrap"` + `columnGap`/`rowGap`. Added `rowLead` (`flexGrow:1, flexShrink:1, flexBasis:0, minWidth:140`) and `rowControls` (`flexShrink:0, marginLeft:"auto"`). Title keeps normal word-wrap (no `numberOfLines` — ND legibility).
- [x] `EditGoalStepRow.tsx`: `[≡][number][title]` in a `rowLead` `View`, `[evidence][reorder]` in a `rowControls` `View`, both inside `rowMain`. Wide screens keep the single-line look; narrow rows wrap the controls to a second line.
- [x] `EditGoalSubStepRow.tsx`: same clustering — `[≡][title]` in `rowLead`, `[evidence][reorder][×]` in `rowControls`.
- [x] Drag handle stays inside the gesture-wrapped body (the new wrappers are entirely within it — no gesture regression; all 46 EditGoalView tests still green).
- [x] Spacing tokens only, no hex; `minWidth:140` is a raw layout dimension consistent with the file's existing `minHeight:44/48`, `width:48`.
...
**Branch:** `feat/issue-459-reorder-smaller-steps` — all 6 steps landed + plan-doc commits, DCO-signed, **not yet pushed**. Full suite green: `type-check` ✅, `lint` ✅ (only pre-existing file-size warnings), `test` ✅ (204 suites / 9778 tests). Step 6 narrow-screen fix verified at 340px in Storybook. **Ready for `/finalize`** (push + PR).
...
- ✅ **Step 6 — Narrow-screen row fix** (D5) — `fix(edit-goal): stop step/sub-step rows crushing the title on narrow screens` (SHA 85803730). Flex-wrapped the trailing controls into a `rowControls` cluster (shared by both `EditGoalStepRow` and `EditGoalSubStepRow`); added a render-integrity regression test. User OK'd folding into #459 (also fixes #445's parent rows). Verified at 340px in Storybook.
- ⬜ **Final validation & PR** — all gates green (above). Remaining: `/finalize` (push + PR). Manual Storybook eyeball still worth doing for the two gesture-only intent items (drag stays within parent; parent card doesn't co-drag — gestures are mocked in Node) and the `subStepRowDragging` border note below.
