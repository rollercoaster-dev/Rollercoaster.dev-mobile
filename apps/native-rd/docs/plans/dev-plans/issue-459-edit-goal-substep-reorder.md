...
### Step 6: Fix the narrow-screen row crush (D5)

**Files**: `EditGoalView/EditGoalStepRow.tsx`, `EditGoalView/EditGoalSubStepRow.tsx`, `EditGoalView/EditGoalView.styles.ts`
**Commit**: `fix(edit-goal): stop step/sub-step rows crushing the title on narrow screens`
**Status**: ⬜ Not started — surfaced by review screenshots on 2026-07-07 (see Discovery Log).

**Problem** (confirmed in code): `rowMain` and `subStepRow` are single, non-wrapping flex rows — `[≡][number?][title (flex:1)][evidence chip][reorder buttons][× (sub-step)]`. The trailing controls use RN's default `flexShrink: 0`, so on a narrow viewport the title (`flex:1`) absorbs all the negative free space and collapses to **one character per line**; once it hits zero width, the sum of the non-shrinking controls sets a minimum row width that exceeds the viewport, stretching the whole `body` column past the screen edge (which is why the header and goal-title card also clip on the right). One root cause, both symptoms. Worst on a two-evidence-type step with the ↑/↓ fallback showing (reduced-motion / screen-reader) = four fixed trailing items. #459 did not create this — it is a latent #445 defect — but #459's new sub-step ↑/↓ buttons reproduce the same crush on sub-step rows, so the fix lands here to keep this PR's own deliverable passing the visual gate (and fixes the parent rows in the same shared-style change).

**Changes** (D5 — flex-wrap the trailing controls, never truncate the title):

- [ ] `EditGoalView.styles.ts`: `rowMain` and `subStepRow` gain `flexWrap: "wrap"` and `rowGap`. Add a `rowLead` cluster style (`flexDirection: row`, `alignItems: center`, `gap`, `flex: 1`, `minWidth: <token-based floor small enough for a ~320pt screen minus the body padding, e.g. theme.space-derived ~120>`) and a `rowControls` cluster style (`flexDirection: row`, `alignItems: center`, `gap`, `marginLeft: "auto"` so it right-aligns on the wrap line). Title text keeps normal word-wrap (no `numberOfLines` — ND legibility, [[project_every_step_requires_evidence]]-adjacent readability principle).
- [ ] `EditGoalStepRow.tsx`: wrap `[≡][number][title]` in a `rowLead` `View` and `[evidence][reorder]` in a `rowControls` `View`, both inside `rowMain`. On wide screens the two clusters sit on one line exactly as today; when the row can't fit, `rowControls` wraps to a second line under the title instead of crushing it.
- [ ] `EditGoalSubStepRow.tsx`: same clustering — `[≡][title]` in `rowLead`, `[evidence][reorder][×]` in `rowControls`.
- [ ] Verify the drag handle stays inside the gesture-wrapped body (Step 1 scoped the parent gesture to the body; the new `rowLead`/`rowControls` wrappers stay entirely within it — no gesture regression).
- [ ] No token/hex changes beyond spacing tokens; verify against `AllThemesMatrix`.
...
- [ ] **Step 6 (narrow-screen fix):** add a component test that a step with two evidence types **and** the ↑/↓ fallback active (`mockAnimationPref = "none"`) still renders its full title text (assert the title node is present and un-truncated) — a regression guard against the crush. Layout width itself isn't measurable in the Node renderer, so the primary check is a Storybook eyeball at a narrow canvas (resize the browser panel / use a phone-width viewport): confirm the title wraps by words and the controls drop to a second line, and the header + goal-title card no longer clip on the right. Verify across `AllThemesMatrix`.
...
- [2026-07-07] **Narrow-screen row crush found in review (→ Step 6, D5).** Screenshots at a phone-width canvas showed step/sub-step titles collapsing to one character per line and the header/goal-title card clipping off the right edge. Root cause (confirmed in `EditGoalStepRow.tsx` L199-253 / `EditGoalSubStepRow.tsx` L188-247): a single non-wrapping flex row where the title (`flex:1`) is the only shrinkable child, so it absorbs all overflow and then forces the body column wider than the viewport. It is a **pre-existing #445 defect** (reproduces on parent rows with no #459 code), but #459's new sub-step ↑/↓ controls reproduce it on sub-step rows too, so the shared-style fix is folded into this PR rather than punted — otherwise #459 would ship a visibly-broken deliverable (fails the visual-gate principle: green tests ≠ right design). Scope note for the user: this touches the shared parent-row layout, so the PR now also fixes #445's rows. Confirm this bundling is acceptable, or split Step 6 into its own issue/PR.

## Progress & Resume (for a fresh context)

**Branch:** `feat/issue-459-reorder-smaller-steps` — Steps 1-5 landed + plan-doc commits, DCO-signed, not yet pushed. Full suite green as of Step 5: `type-check` ✅, `lint` ✅ (only pre-existing warnings in untouched files), `test` ✅ (204 suites / 9777 tests). **Now blocked on Step 6** (narrow-screen row crush found in review — see Discovery Log 2026-07-07 / D5). Do **not** `/finalize` until Step 6 lands and its regression test + Storybook eyeball pass.
...
- ⬜ **Step 6 — Narrow-screen row fix** (D5). Flex-wrap the trailing controls into a `rowControls` cluster so a narrow row drops them to a second line instead of crushing the title; add the regression test. Applies to both `EditGoalStepRow` and `EditGoalSubStepRow` (shared style). See the Step 6 section above. **Awaiting user OK on the scope note** (this fix touches the shared parent-row layout, i.e. also fixes #445's rows).
- ⬜ **Final validation & PR** — after Step 6: full `type-check` / `lint` / `test`, then `/finalize` (push + PR). Manual Storybook eyeball still worth doing for the two gesture-only intent items (drag stays within parent; parent card doesn't co-drag), the narrow-canvas layout (Step 6), and the `subStepRowDragging` border note below — gestures are mocked in Node so those aren't unit-covered.
