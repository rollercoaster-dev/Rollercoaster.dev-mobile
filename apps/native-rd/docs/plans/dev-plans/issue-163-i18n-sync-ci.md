...
| D9  | Split into two jobs (`sync-pr` and `sync-dispatch`) with per-job permissions, workflow-level `permissions: {}`                                         | Single job with workflow-level `contents: write` + `pull-requests: write`     | **Resolved 2026-05-25 (Copilot review):** `pull-requests: write` is only needed by the `workflow_dispatch` path that calls `gh pr create`. Keeping that scope on the `pull_request` job — which receives `OPENROUTER_API_KEY` and runs PR-author-controlled code paths — is broader than necessary. Splitting puts `sync-pr` on `contents: write` only (push the de/ commit back to the PR branch) and isolates `pull-requests: write` to `sync-dispatch`, which only runs from a trusted ref under `workflow_dispatch`. Defence-in-depth on top of the D2 fork guard and D8 path guard. |
...
- [x] Set top-level permissions: `contents: write` (needed for git push and branch creation), `pull-requests: write` (needed for `gh pr create` on `workflow_dispatch`). All other permissions default to `none`. **Revised 2026-05-25 (Copilot review, D9):** top-level is now `permissions: {}` and the two scopes are declared per-job — `sync-pr` gets `contents: write` only, `sync-dispatch` gets both. The single-job structure described in the rest of this step has been split accordingly; the step contents (guards, checkout, diff, commit/PR-open logic) carry over to the two jobs unchanged.
- [x] One job `sync` with:
