...
- [x] In `common.json` (en): add `"paused": "Paused"` under `stepCard.status`.
- [x] In `common.json` (de): add `"paused": "Pausiert"`. _(Pseudo regenerated via
      `bun run gen:pseudo` — pseudo is generated from en, not hand-edited.)_
- [x] Widen the `TimelineNode` prop type: `type NodeStatus = StepStatus | "paused"`.
      Add a comment: `// TODO: collapse into StepStatus once the data layer supports paused`.
- [x] Add `paused` branches to `nodeStyle` and `textStyle` arrays (parallel to
      the existing `completed` and `in-progress` branches). _(Also added explicit
      `pending` branches so the new `pendingNode`/`pendingText` styles apply.)_
- [x] Update `content` logic: before the step-number fallthrough, add
      `status === "paused" ? stepStateColorMap["paused"].nodeGlyph : ...`.
- [x] Add a `showStateBadge?: boolean` prop (default `false`) — see **D7**.
...
- [x] Do NOT add the badge to the hitSlop/press area — it is a label, not a target.
...
- [2026-06-29] **Step 4 done & committed** (`5723c064`). Styles resolve through
  the map; 0 `palette.blue600` refs.
- [2026-06-29] **Step 5 done.** Prop type widened to `NodeStatus`; paused branches,
  glyph (⏸ via the map), and the opt-in `showStateBadge` badge wired. **Typed-i18n-key
  fix:** the repo uses i18next typed keys, so `t(badgeI18nKey)` rejected a plain
  `string`. Typed `badgeI18nKey` as the template literal `StepStateBadgeKey =
...
- [2026-06-29] **Pre-existing pseudo drift discovered (NOT fixed here).** Running
  ...
  `pseudoTransform` and never regenerated. That drift is unrelated to #406, so I
  reverted all pseudo files and hand-added only the `paused` key to
  `pseudo/common.json`. **Follow-up owed:** a `bun run gen:pseudo` housekeeping PR
  to resync the pseudo locale (or a CI check that fails when pseudo is stale).
