...
| Fixing `ThemeChipGrid`'s/`ThemeSwitcher`'s identical `accessible={true}` radio-collapse bug | Both components' only screen mounts are removed by this issue (`ThemeChipGrid`) and #416 (`ThemeSwitcher`); #500's own plan already scoped this out for the same reason. | none — components go unmounted, no live surface left to fix |
