...
| Guard against an absent CTA handler (Copilot review, declined) | Optional CTA props (`onAddEvidence` etc.) could render an announced-but-inert button if a consumer omits one. Not triggerable here — the card is unconsumed; the proper fix is a per-state discriminated-union API that makes each view's handlers required, a refactor too large for a review-fix pass. | #377 (wires the card — verify all handlers are passed); revisit required-handler typing if/when a 2nd consumer appears |
