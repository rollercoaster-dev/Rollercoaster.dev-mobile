...
- [x] **Query change (D2):** add `"step.updatedAt"` to `stepsForActiveGoalsQuery`'s `.select([...])` (additive — do NOT change its `orderBy`, which `resolveNextActionableStep` depends on). `activeGoalsQuery` stays `createdAt DESC`; ranking happens in the container.
...
- [2026-06-28] **Step 3b — typing note resolved: no accessor needed.** `typeof activeGoalsQuery.Row` (Evolu `selectAll`) exposes `createdAt`/`updatedAt`, and `step.updatedAt` is typed once added to the steps query's `.select([...])`. `bun run type-check` (incl. `tsconfig.test.json`) is green with the container reading them directly, so the planned "small typed accessor" fallback was unnecessary. The recency key compares them as plain strings via an `isoOrNull(value: unknown)` guard (same-format UTC ISO-8601 sorts lexically == chronologically).
- [2026-06-28] **Step 3b — pseudo locale for the cockpit keys was never generated in Step 2.** The full test suite surfaced a pre-existing red: `i18n/__tests__/locale-parity.test.ts` "goals namespace" failed because `pseudo/goals.json` was missing all six `cockpit.*` keys — Step 2 (`cdab766`) added them to `en/goals.json` but didn't run `bun run gen:pseudo` (D11). Regenerated and staged **only** `pseudo/goals.json`; reverted the generator's unrelated trailing-dot padding drift in `pseudo/{focusMode,completion,editGoal}.json` (key sets unchanged, so their parity tests stayed green) to keep PR A scoped. Shipped as its own commit ahead of the container wiring so each commit leaves the suite green.
...
- [2026-06-28] **Prototype-fidelity pass (PR A) — the structural criteria were met but the cockpit didn't _look_ like the "Goals + Badges C Prototype".** Reviewed the GoalsCockpit web Storybook story side-by-side with the prototype and closed the visual gaps the checked criteria didn't cover: (1) **ring reordered above** the "DO THIS NEXT" overline and sized up for hero presence (`size=124`/`strokeWidth=10` — bumped past the prototype's 104 in a later PR-A commit, "larger cockpit ring"); (2) **▶ play icon** on the Start/Resume `Button` (`icon="▶"`); (3) ring sublabel → new `cockpit.ringSteps` ("`{{completed}} / {{total}} steps`", uppercased via `ProgressRing.styles` `centerSublabel` switched to `variant="mono"` + `textTransform`) so it reads "5 / 9 STEPS" — left the shared `card.progressLabel` (GoalCard/Timeline) untouched; (4) **keep-warm cards 2-up** via a `keepWarmGrid` (row/wrap, `flexBasis:"45%"`) instead of a full-width stack; (5) **"+ New goal"** → dashed-outline ghost `Pressable` (not a text Button); (6) **bespoke empty state** (D-decision reversed per user 2026-06-28) — concentric-circle `react-native-svg` reticle in a dashed frame + "Pick something to prove" + warmer body + blue CTA, replacing the generic `EmptyState`; copy in `emptyState.{title,body,cta}`. **Header (container):** moved `ScreenHeader` into `GoalsCockpitContainer` so it can show **"Today" + a live goal count** (`todayTitle` / pluralized `goalCount`) when goals exist, falling back to "Goals" when empty (matches prototype Cockpit vs Empty). Trade-off: header now sits under the Suspense boundary, so it's absent during the (sub-ms, local-first) load. All colors are theme tokens — verified clean in `light-default` + `dark-default`. `bun run gen:pseudo` regenerated `pseudo/goals.json` only (reverted unrelated padding drift). type-check ✅, lint ✅ (0 errors), tests ✅ (63 across GoalsScreen/GoalsCockpit/ProgressRing/locale-parity).
- [2026-06-28] **Web Storybook had no i18n provider — the published 7-theme review surface rendered raw key paths.** `.storybook-web/preview.tsx` configured Unistyles + fonts + the theme toolbar but never initialised `react-i18next`, so every `t()`-driven story showed "goals:cockpit.resume" etc. (keys were correct in `en/goals.json`; they resolved only in-simulator). Wrapped the story decorator in `<I18nextProvider i18n={i18n}>` importing the real app i18n (`../src/i18n`; `expo-localization`'s web build `ExpoLocalization.js` resolves under Vite). This is load-bearing for D13 — the BadgesWall 7-theme check in PR B reads from this same surface and would otherwise misrepresent translated copy.
