...
- [x] Leaving an armed target row disarms it immediately; movement within the same row keeps it armed so normal finger jitter does not erase the sustained indicator. _(disarm keyed on hovered-row change via `hoverStepIdRef`)_
