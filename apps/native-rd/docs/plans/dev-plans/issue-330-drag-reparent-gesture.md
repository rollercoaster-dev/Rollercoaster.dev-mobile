...
- [x] Dragging a root that has substeps over any row in another root's group reorders the complete parent+children block; its children retain their parent IDs and the destination group receives a dashed outline.
...
- [x] Leaving an armed root's measured row disarms it immediately; movement within the row keeps it armed, while the gaps outside its bounds remain positional before/after insertion targets. _(disarm keyed on hovered row + dwell-zone refs)_
...
- [2026-06-20 PM] **Top-root insertion vs. dwell correction.** A root dragged above the first row could dwell-arm that row while passing over it, causing the armed demote result to override the positional reorder. Dwell now requires the dragged row's centre to be within the target's measured bounds; moving into the gap above the first root cancels the dwell timer/armed state and preserves the insertion slot.
- [2026-06-20 PM] **Dwell hit-zone correction.** Restricting dwell to the middle 60% made valid downward targets too difficult to reach with dynamic row geometry. The full measured row is now the dwell target; only positions outside its bounds act as insertion zones. This preserves top insertion without shrinking the visible target.
- [2026-06-20 PM] **Root-group reorder correction.** A root with children was still classified as a single row, so hovering another group's child looked like an illegal attempt to demote a parent and returned `none`. Parent roots now reorder as complete blocks: hovering either a destination root or any of its children targets that root group, dispatches one root-ordinal reorder, and outlines the full destination group. Children remain attached through their unchanged `parentStepId` values.
