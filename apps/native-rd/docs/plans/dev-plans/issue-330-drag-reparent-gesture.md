...
- [x] Dragging a leaf child step to a position above all of its parent's siblings promotes it to a root step (parentStepId cleared, no data corruption). _(classifyDrop dispatch + EditModeScreen ordinal normalization)_
- [x] Hovering over a childless root step for ~200-250 ms while dragging a leaf step causes that target to visually arm (grow + pulse for full/reduced-motion users; static bold border for `animationPref === "none"`). _(`DWELL_ARM_MS = 220`; DraggableStepItem `scaleArmed` + `armedTargetItem`)_
...
- [x] Any drag movement after arming disarms the target immediately (no unintended demote). _(disarm keyed on hovered-row change via `hoverStepIdRef`)_
...
- [x] Dragging over a non-root step as a dwell target is refused (snap back). _(arm condition requires `parentStepId == null` + childless)_
...
| --- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| D8  | Vertical-only drag + dwell-to-demote (settled in #290 plan)                                                                                                                                                       | x-axis indent; explicit buttons only            | Shipped vertical DnD before; dwell resolves the only ambiguous positional case (childless root target)                                                                        |
...
| D15 | `armedTargetId` stored as React state in `StepList`; passed to `DraggableStepItem` as a prop for the highlight animation                                                                                          | Store on a shared value                         | The armed target identity only changes on dwell events (not per-frame), so React state re-render is fine; it also makes unit-testable component assertions straightforward    |
...
- [ ] Add props:
  ...
  canNestUnder?: boolean;   // true if this step is a childless root that has ≥1 eligible target root (can be demoted)
...
- [ ] New describe block `SR nest/un-nest controls`:
  - With accessible controls shown, a childless root step that has ≥1 eligible target root renders a "Nest under…" trigger button (`nestUnderTrigger`).
  - Pressing the trigger opens the picker; the picker lists exactly the eligible target roots (childless, excluding self), each row labelled with its title.
  - Selecting a picker row calls `onReparentStep(stepId, targetId)` with the chosen target id.
  - A childless root with no eligible targets does **not** render the trigger.
...
- [ ] Manual QA (device required — see issue "Manual device QA" scope item):
  ...
  - Dwell-demote: hover leaf root over a childless root ~220 ms → arm feedback → release.
