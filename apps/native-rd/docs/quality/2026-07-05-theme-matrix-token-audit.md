...
**Matrix conversions (rec. #2) — 8 of the 10 converted matrices work.** SettingsDensityRows, SettingsThemeSection, TimelineStep, TimelineBreakdownBar, TimelineNode, EvidenceTypePicker, FocusCurrentTaskCard, BadgeWallCell all now render the real component correctly per theme. So HIGH-2 (SettingsDensityRows "null matrix") is **resolved**, not a live issue anymore.

**EditGoalView (MEDIUM-1) — root cause was misdiagnosed.** It is _not_ a "null matrix" and _not_ the `useUnistyles` styling trap — EditGoalView is styled with `StyleSheet.create` like the working TimelineStep. The real cause: **EditGoalView re-renders after mount** (`useAnimationPref` + its AccessibilityInfo screen-reader/reduce-motion probes resolve async and `setState`). On web, `<ScopedTheme>` applies the scope **only during the initial render pass**; any later re-render recomputes `StyleSheet.create` styles against the _active_ global theme, so all 7 cells silently revert. This is an inherent web limitation of `<ScopedTheme>` for re-rendering components — the 8 working matrices only work because they are pure prop-driven and never re-render.

**Resolution:** EditGoalView's `AllThemesMatrix` renders a single instance + a note directing reviewers to the **theme toolbar** (which sets the global theme — the component honours it correctly, verified in dark-default). A live per-cell 7-up is not achievable for it on web without an iframe-per-theme approach (deferred; not built).

**Latent risk this exposes:** the inline-`ScopedTheme` matrix pattern is silently fragile — if any of the 8 currently-working components later gains a post-mount re-render (e.g. wiring real data/subscriptions at [Integrate]), its matrix will revert to the active theme the same way, with no error. A future robustness pass could move matrices to iframe-per-theme; not scoped here.

**Token fixes (rec. #1, #4, #5) — shipped and gate-clean.** `palette.white` → `theme.colors.accentPurpleFg` (HIGH-1) in BadgeCard, BadgeWallCell, BadgesWall; `palette.yellow300` → `theme.colors.accentYellow` (+ `accentYellowFg` ink for the goal node) across the 6 sites; BadgesWall's intentional fixed-dark colors hoisted behind one documented `eslint-disable` block; `no-raw-colors` hardened to cover screens + `palette.*` member expressions. `type-check` clean, `eslint` on all `*.styles.ts` = 0 errors, rule test 66/66.
