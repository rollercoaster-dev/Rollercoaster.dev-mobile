...
7. **C·B data source.** _As of this review_ the step table had **no dependency or due-date fields** (`src/db/schema.ts`, `step`); `afterStep`/`waitingOn`/`dueDate` were story-only props. #378 declares itself "wiring + queries only" — there is nothing to query. Either the band ships dormant (document that) or a schema issue is missing. **Resolved:** #454 added the four columns (`afterStepId`, `waitingOnLabel`, `waitingOnExpectedAt`, `dueAt`) plus `resolveStepDependencyBand`, and #377/#378 wired the read side — but no authoring UI was in scope, so **the band did ship dormant**: it renders only the facts a step carries, and nothing can set them. Documented here; epic #570 (filed 2026-08-11) builds the authoring surface.
