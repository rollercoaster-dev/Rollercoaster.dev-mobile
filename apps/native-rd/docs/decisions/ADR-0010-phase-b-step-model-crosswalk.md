...
| **F: Mid-work capture**             | Phase B gives users a low-friction landing place for structure or insight discovered during work.                                            | F is not completion evidence and not a goal review. It also does not require this ADR to choose a modal, tap count, capture medium, or navigation path.   | The capture surface must preserve bursts of thought before a context switch. Exact interaction design belongs in later implementation planning.                                                  |
