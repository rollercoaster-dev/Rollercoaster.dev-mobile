...
| Screen that mounts any of it                                         | **None**                                      | `StepOverviewCard` / `StepCardTopBand` are reached only via `StepCard.tsx:185,316`, and since #455 deleted `CardCarousel` — its last importer — `StepCard` is reached only from its own story and test |
