...
This brief consolidates the initial exploration. File export/import, fixed media limits, and a final-badge-only flow were tentative suggestions, not accepted requirements. The 2026-09-23 workshop defines a learner-prepared review view, complete claim disclosure, selected evidence, comments and replies, confirmed attribution, and scoped recognition. Transport and exact interaction and protocol choices remain open. The vision supersedes earlier assumptions about fixed validation gates and automatic evidence retention.
...
| Step recognition      | Separate the underlying credential from its display under a step or in the badge collection. Determine issuance timing for revisable learner commitments.                                                   |
...
| Endorsements          | Investigate OB3 EndorsementCredentials for specific reviewed claims; exact mapping remains open. Reuse the current standards work without assuming badge issuance also proves endorsement interoperability. |
...
Peer validation can be a revisable learner-selected commitment, not a fixed external gate. The learner owns completion; the peer owns their statement. Implementation alignment with the existing Phase B rules needs an explicit decision. The governing [ADR-0013](../decisions/ADR-0013-phase-b-consolidated-position.md) says: “No auto-judgment — forbidden, always.” This research does not amend an accepted ADR or turn a structural dependency into a completion gate.
...
1. Prototype the agreed review view and test brief, interrupted, and repeated in-person encounters. Keep access duration open; do not assume viewing authorizes permanent evidence copies.
2. Compare technology candidates against complete claim disclosure, selected evidence, comments and replies, attribution confirmation, private drafts, and scoped recognition. Investigate the strong offline preference on real devices without assuming it is feasible.
3. Design candidate step/credential/version mappings, issuance timing for revisable commitments, and standalone badge presentation. Preserve recognition of the original attempt.
4. Test independent endorsement interoperability and exchange behavior, including revised claims, duplicate responses, interrupted reviews, and unavailable network checks.
5. Workshop mentor progression and rewards separately; in-app discovery and badge-tree mapping are future work, not dependencies of the initial encounter.
